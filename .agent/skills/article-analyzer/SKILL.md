...
description: 文章深度分析技能。当用户要求分析文章、解读文章内容、评估文章观点或学习写作技巧时使用此技能。适用于任何类型的文章分析需求，包括论述文、评论文、新闻报道等。
---
# 文章深度分析技能

## 任务目标

- 本 Skill 用于：深度分析文章内容，提供结构化的解读与批判性审视
- 能力包含：
  - 核心论点提取与论证逻辑分析
  - 背景语境与作者意图解读
  - 批判性审视与价值提取
...
2. **联网检索（如需要）**
   ...
   - 参考[联网检索要求](#联网检索要求)，判断是否需要检索
   - 如需检索，执行事实核查或背景补充
   - 在分析中标注核查结果与信息来源
   - 收集所有引用的参考资料信息（标题、URL、摘要）
3. **执行五维分析**

   - 按照[分析框架](#分析框架)，逐一完成五个维度的分析
   - 引用文章原文佐证观点
   - 避免空泛概括，提供具体洞察
4. **生成分析报告**

   - 按标准格式输出完整分析
   - 保存为 `analysis.md` 文件到文章目录
   - 若进行了联网检索，保存参考资料为 `references.md` 文件

### 调用模式

当被其他技能调用时，遵循调用方的特殊要求：
...
```
analyzer/20260108_ai-future/
├── source-1.md       # 素材
├── analysis.md       # 分析结果
└── references.md     # (可选) 联网检索的参考资料
```
...
- 引用原文时标注出处，便于验证
- 批判性审视应客观公正，避免主观偏见
- 充分利用智能体的语言理解、推理分析和联网检索能力
- 避免为分析任务编写脚本，这是智能体的强项
